...
#'
#' \code{mitoFeatureReduction} takes a MultiAssayExperiment
...
#'
#' #df <- mitoFeatureReduction(mitoMAE2, nPC = 3, nTSNE = 2, perplexity = 30)
#'
#' @export
setGeneric(name = "mitoFeatureReduction", def = function(mitoMAE, nPC = 10, nTSNE = 2, perplexity = 30)
...
setMethod("mitoFeatureReduction", signature("SummarizedExperiment", "ANY", "ANY", "ANY"),
          definition = function(mitoMAE, nPC = 10, nTSNE = 2, perplexity = 30){

  mat <- assays(mitoMAE)[["frequency"]]
  ...
  df <- data.frame(colData(mitoMAE), tSNEdf, pcdf)
  return(df)
})
