...
samtools mpileup -d8000 -r chrM -l $out/QChetvars.bed -Q20 -t AD,ADF,ADR --skip-indels -b $bamlist -v > $calls/All.SNPs.vcf.gz
...
cat $calls/All.SNPs.MNP.txt | sed -e 's/INFO\///g' -e 's/\.bam//g' -e 's/,/\t/g' | awk 'BEGIN {OFS=FS="\t"} {print $2, $3, $4, $6}' > $calls/All.SNPs.MNP.clean.txt
